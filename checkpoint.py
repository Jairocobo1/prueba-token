...
def Ret_Pregunta01():
    ...
    '''
    #Tu código aca:
    df = pd.read_csv('CP05_FT10_03/datasets/Fuentes_Consumo_Energia.csv', header=0)
    ContadorRCol = df['Entity'].str.contains('Colombia').value_counts()[True]
    ContadorRMex = df['Entity'].str.contains('Mexico').value_counts()[True]
    TuplaRegis = (ContadorRCol,ContadorRMex)
    return (TuplaRegis)
...
def Ret_Pregunta04():
    ...
    df = df[df['Entity']== 'World' ]                                #dfxFW seleccionamos filas de Entity con palabra World 
    df= df[df['Year']==2019]                                #dfxFW seleccionamos filas de Year  con entero 2010
    df =df[['Entity','Year','Coal_Consumption_EJ','Gas_Consumption_EJ','Oil_Consumption_EJ','Geo_Biomass_Other_TWh', 'Hydro_Generation_TWh','Nuclear_Generation_TWh', 'Solar_Generation_TWh', 'Wind_Generation_TWh']]

    valor1enTW =df.iloc[0,2:5].sum() * 277.778,2 
    valor1enTW =valor1enTW + df.iloc[0,5:10].sum()
    return round(valor1enTW[0],2)
...
def Ret_Pregunta05():
    ...
    '''
    #Tu código aca:
    df = pd.read_csv('CP05_FT10_03/datasets/Fuentes_Consumo_Energia.csv')
    #df = df[['Entity','Year','Hydro_Generation_TWh']] #filtrando columnas con un solo df
    df = df[(df['Entity']== 'Europe')&(df['Hydro_Generation_TWh']>= 502)]                  #filtrando para mas de 1 filas df = df[df['Entity'].isin(['Europe','Colombia'])] 
    df = df.set_index('Year')
    #df = df[['Hydro_Generation_TWh']]
    max = int( df['Hydro_Generation_TWh'].idxmax()) #argmax()
    return max
...
def Ret_Pregunta07():
    ...
    '''
    #Tu código aca:
    df = pd.read_csv('CP05_FT10_03/datasets/GGAL - Cotizaciones historicas.csv')
...
def Ret_Pregunta08():
    ...
    #Tu código aca:
    df = pd.read_csv('CP05_FT10_03/datasets/Fuentes_Consumo_Energia.csv')
    return (len(pd.unique(df['Entity'])))
    #return 'Funcion incompleta'
...
def Ret_Pregunta09():
    ...
    #Tu código aca:
    df1 =pd. read_csv('CP05_FT10_03/datasets/Tabla1_ejercicio.csv',sep=';')
    df2 =pd. read_csv('CP05_FT10_03/datasets/Tabla2_ejercicio.csv',sep=';')
    df3 = pd.merge(df1,df2, on='pers_id', how= 'outer' )
    df3.drop_duplicates(inplace=True)
    score_promedio_femenino = round(float(df3['score'][df3['sexo'] =='F'].mean()),2)
    score_promedio_masculino = round(float(df3['score'][df3['sexo'] =='M'].mean()),2)
    return tuple ((score_promedio_femenino, score_promedio_masculino))
    #return 'Funcion incompleta'
